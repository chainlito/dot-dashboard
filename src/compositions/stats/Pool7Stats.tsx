...
const Pool2Stats = ({ tokenPrice, account }: Props) => {
  ...
  useEffect(() => {
    web3client.getTotalSupply(web3client.pool7Contract)
			.then(res => setTotalStaked(res));
		if (account) {
			web3client.getBalance(web3client.pool7Contract, account.address)
				.then(res => setStaked(res));
			web3client.poolGetEarned(web3client.pool7Contract, account.address)
  ...
  return (
    <React.Fragment>
      <h2 className='mb-20 text-medium text-center'>{Config.MemeToken.name} staking pool</h2>
	  	<div className='card card-h white'>
      	<div className='text-small mb-10'>PRICES <span className='ybullets'> • • • • • • • • • • • • • • • • • • • • • • • • • •</span></div>
	  		<div className='flex-h'>
      		<div className='text-tiny wp-50'>{`1 ${Config.MemeToken.symbol} =`}</div>
